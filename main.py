...
# def add_hyperlink_to_title(df, config):
#     """Add hyperlinks to query titles using query_id"""
#     # Check for different possible column names for the title
#     title_column = None
#     if 'title' in df.columns:
#         title_column = 'title'
#     elif 'Query Name' in df.columns:
#         title_column = 'Query Name'
    
#     if 'query_id' not in df.columns or title_column is None:
#         return df
...
#     df[title_column] = df.apply(
#         lambda row: f'<a href="{urls[row.query_id]["url"]}" target="_blank">{row[title_column]}</a>' 
#         if row.query_id in urls and pd.notna(row.query_id) 
#         else row[title_column], 
#         axis=1
#     )
    
#     logging.info('Added hyperlinks to query titles')
#     return df
...
#         # Add hyperlinks to titles if query_id and title columns exist
#         if 'query_id' in df.columns and 'title' in df.columns:
#             df = add_hyperlink_to_title(df, config)

#         # Add URL column if query_id exists (for backward compatibility)
#         if 'query_id' in df.columns and 'title' not in df.columns:
...
#                 "query": """SELECT 
#                         csq.identifier as query_id,
#                         csq.title,  
#                         CONCAT(usr.first_name, ' ', usr.last_name) AS name,  
#                         ROUND(MAX(logs.time_to_finish) / 1000, 0) AS time_taken
#                     FROM 
#                         catissue_query_audit_logs logs  
#                     INNER JOIN 
#                         catissue_user usr ON logs.run_by = usr.identifier
#                     INNER JOIN 
#                         catissue_saved_queries csq ON csq.identifier = logs.query_id
#                     WHERE 
...
#                         AND logs.query_sql NOT LIKE '%limit 0, 101%'
#                     GROUP BY 
#                         csq.identifier, csq.title, name
#                     ORDER BY 
#                         time_taken DESC
#                     LIMIT 5;
#                     """,
#                 "output_filename": "slowest_running_queries.html",
#                 "drop_columns": ["query_id"]  # Hide the query_id column but keep it for processing
#             },
#             {
#                 "title": "Top 5 most run saved queries.",
#                 "query": """ SELECT csq.identifier as query_id,
#                                    title, 
#                                    CONCAT(first_name, ' ', last_name) AS name, 
#                                    ROUND(COUNT(*)/2,0) AS count 
#                             FROM catissue_query_audit_logs logs 
#                             JOIN catissue_user user ON logs.run_by = user.identifier
#                             JOIN catissue_saved_queries csq on
#                             csq.identifier = logs.query_id
#                             WHERE query_id IS NOT NULL 
#                                  AND query_sql NOT LIKE '%limit 0, 101%'
#                                  GROUP BY csq.identifier, title, name
#                             ORDER BY count DESC 
#                             LIMIT 5;""",
...
#                     WHERE query_id IS NOT NULL AND
...
def generate_html_report(query, output_filename, config, db_connection, drop_columns=None):
    try:
        ...
        # Add hyperlinks to titles if query_id and title columns exist
        if 'query_id' in df.columns and ('title' in df.columns or 'Query Name' in df.columns):
            df = add_hyperlink_to_title(df, config)

        # Remove the URL column addition - we don't need it anymore since titles are hyperlinked
        ...
        # Add CSS to center only the column headers
        html_table = f"<style>th {{ text-align: center; }}</style>{html_table}"
        ...
        queries = [
            {
                ...
                "query": """SELECT 
                    ...
                    WHERE 
                        logs.query_id IS NOT NULL  
                        AND logs.time_of_exec >= '2025-07-25 00:00:00'
                        AND logs.time_of_exec <= '2025-07-25 23:59:59'
                        AND logs.query_sql NOT LIKE '%limit 0, 101%'
                    GROUP BY 
                        csq.identifier, csq.title, name
                    ORDER BY 
            ...
            },
            {
                ...
                "query": """SELECT 
                    ...
                    JOIN 
                        ...
                        ON user.identifier = logs.run_by 
                    WHERE query_id IS NOT NULL AND
                           logs.time_of_exec >= '2025-07-25 00:00:00'
                            AND logs.time_of_exec <= '2025-07-25 23:59:59'
                        AND query_sql NOT LIKE '%limit 0, 101%'
                    GROUP BY 
                        run_by
            ...
            },
            {
                ...
                "query": """SELECT
                        ...
                        JOIN
                            catissue_user usr ON usr.identifier = logs.user_id
                        WHERE
                             logs.CALL_START_TIME >= '2025-07-25 00:00:00'
                            AND logs.CALL_START_TIME <= '2025-07-25 23:59:59'
                        ORDER BY
                            TIMESTAMPDIFF(SECOND, logs.call_start_time, logs.call_end_time) DESC
                        LIMIT 5;
                        """,
